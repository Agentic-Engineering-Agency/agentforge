...
The current release line is `v0.12.23`.

## Prerequisites

- `main` contains the release commit.
- GitHub repository secret `NPM_ACCESS_TOKEN` is configured.
...
4. Create or publish the GitHub release for `v0.12.23`.
5. The `Publish Packages` GitHub Actions workflow publishes any missing package versions in order: core, runtime, cli.

## Manual Retry

If the publish workflow needs to be retried after the tag already exists, run `Publish Packages` with the `version` input set to `v0.12.23` or `0.12.23`.
...
- `NPM_ACCESS_TOKEN` is required for the publish step; without it the workflow exits with a clear error.
